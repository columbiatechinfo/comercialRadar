...
var SEEK = (function () {
  ...
  var S = {
    ...
    /* ONDE (16/09/2026): estado, cidade e bairro */
    fUF: 'todos', fCid: 'todos', fBai: 'todos',
  ...
  function passa(c, sem) {
    if (sem !== '#uf' && S.fUF !== 'todos' && c.uf !== S.fUF) { return false; }
    if (sem !== '#cid' && S.fCid !== 'todos' && c.cidade !== S.fCid) { return false; }
    if (sem !== '#bai' && S.fBai !== 'todos' && c.bairro !== S.fBai) { return false; }
  ...
  function filtra() {
    ...
    var r = {vistos: [], ia: {}, dec: {}, qual: {}, fs: {}, ft: {}, seg: {}, vis: {}, tIA: 0, tDec: 0, tQual: 0,
             uf: {}, cid: {}, bai: {}, tUF: 0, tCid: 0, tBai: 0};
    FONTES_DADO.forEach(function (f) { r.fs[f.id] = 0; r.ft[f.id] = 0; });
    for (var i = 0; i < S.casos.length; i++) {
      var c = S.casos[i];
      if (passa(c, null)) { r.vistos.push(c); }
      if (passa(c, '#uf')) { r.uf[c.uf || '?'] = (r.uf[c.uf || '?'] || 0) + 1; r.tUF++; }
      if (passa(c, '#cid')) { r.cid[c.cidade] = (r.cid[c.cidade] || 0) + 1; r.tCid++; }
      if (passa(c, '#bai')) { r.bai[c.bairro] = (r.bai[c.bairro] || 0) + 1; r.tBai++; }
  ...
  function pintaFila() {
    ...
    /* ONDE: as opções são as que existem com os outros recortes valendo (a cidade depende do estado, o bairro da
     * cidade); a escolhida continua na lista mesmo zerada, para não sumir debaixo de quem escolheu */
    function opLocal(conta, total, escolhida, rotTodos, formata) {
      var ids = Object.keys(conta).filter(function (k) { return k && conta[k] > 0; });
      if (escolhida !== 'todos' && ids.indexOf(escolhida) < 0) { ids.push(escolhida); }
      ids.sort(function (a, b) { return a.localeCompare(b, 'pt-BR'); });
      return [{id: 'todos', rot: rotTodos, n: total}].concat(ids.map(function (k) {
        return {id: k, rot: formata ? formata(k) : k, n: conta[k] || 0};
      }));
    }
    $('#f-uf').innerHTML = seletor('f-sel-uf', 'estado', null, S.fUF, opLocal(r.uf, r.tUF, S.fUF, 'todos'));
    $('#f-cid').innerHTML = seletor('f-sel-cid', 'cidade', null, S.fCid, opLocal(r.cid, r.tCid, S.fCid, 'todas'));
    $('#f-bai').innerHTML = S.fCid === 'todos'
      ? seletor('f-sel-bai', 'bairro', null, 'todos', [{id: 'todos', rot: 'escolha a cidade', n: r.tBai}])
      : seletor('f-sel-bai', 'bairro', null, S.fBai, opLocal(r.bai, r.tBai, S.fBai, 'todos'));
    $('#f-sel-bai').disabled = S.fCid === 'todos';
  ...
  function descreveFiltros() {
    var f = [];
    if (S.fUF !== 'todos') { f.push('estado: ' + S.fUF); }
    if (S.fCid !== 'todos') { f.push('cidade: ' + S.fCid); }
    if (S.fBai !== 'todos') { f.push('bairro: ' + S.fBai); }
  ...
  function liga() {
    ...
    document.addEventListener('change', function (ev) {
      var id = ev.target.id;
      if (id === 'f-sel-uf') { S.fUF = ev.target.value; S.fCid = 'todos'; S.fBai = 'todos'; refiltra(); }
      if (id === 'f-sel-cid') { S.fCid = ev.target.value; S.fBai = 'todos'; refiltra(); }
      if (id === 'f-sel-bai') { S.fBai = ev.target.value; refiltra(); }
