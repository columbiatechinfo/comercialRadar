...
var GESTAO = (function () {
  ...
  function linkSeek(lig) {
    /* REGEX EM TEXTO, NÃO LITERAL (16/09/2026): o `PrefixoPublico` (server.py) troca "(/gestao" por
     * "(/seek/gestao" no código servido, e o literal virava `/seek/gestao\/?$/` — regex inválida que derrubava
     * o script inteiro da gestão na URL pública. */
    var base = window.location.pathname.replace(new RegExp('gestao/?$'), '');
  ...
  function pintaResumo() {
    ...
    var acertosIA = ['aprovar', 'campo', 'revisar'].reduce(function (s, a) { return s + ((t.vigentes || {})[a] || 0); }, 0);
    ...
    $('#g-kpis').innerHTML =
      ...
      + kpi('Acurácia da IA', t.vigentes_total ? pct(acertosIA / t.vigentes_total) : '—',
            num(acertosIA) + ' de ' + num(t.vigentes_total) + ' · ' + num((t.vigentes || {}).rejeitar) + ' rejeitadas',
            'nome e conta dados pelo dono do produto (18/09/2026): (aprovadas + campo + revisão) ÷ ligações decididas '
            + 'pelas pessoas — só a rejeição conta como erro da IA')
